...
main = xmonad $ ewmh desktopConfig
  ...
  , logHook = logHook desktopConfig >> ewmhDesktopsLogHook
  , startupHook = ewmhDesktopsStartup >> startupHook desktopConfig >> spawn "compton"
  , terminal = "urxvt"
  , borderWidth = 1
...
-- Enable Compton's transparency effects
myStartupHook = spawn "compton"

spawnOne "/bin/bash ~/.xmonad/startup.sh"
